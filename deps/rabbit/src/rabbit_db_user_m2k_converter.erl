...
copy_to_khepri(rabbit_user = Table, Record, State) ->
    StoreId = rabbit_db_m2k_converter:get_store_id(State),
    ...
    rabbit_db_m2k_converter:with_correlation_id(
      fun(CorrId) ->
              ...
              ?LOG_DEBUG(
                 ...
                 [Table, Path, CorrId],
                 #{domain => ?KMM_M2K_TABLE_COPY_LOG_DOMAIN}),
              khepri:put(StoreId, Path, Record, Extra)
      end, State);
copy_to_khepri(rabbit_user_permission = Table, Record, State) ->
    #user_permission{
...
copy_to_khepri(rabbit_topic_permission = Table, Record, State) ->
    #topic_permission{
    ...
    rabbit_db_m2k_converter:with_correlation_id(
      fun(CorrId) ->
              ...
              ?LOG_DEBUG(
                 ...
                 [Table, Path, CorrId],
                 #{domain => ?KMM_M2K_TABLE_COPY_LOG_DOMAIN}),
              khepri:delete(StoreId, Path, Extra)
      end, State);
